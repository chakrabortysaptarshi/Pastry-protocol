defmodule PastryNode do
    ...
    def generateActors(index, totalNodes, routingTable) do
        if index <= totalNodes do
            ...
            {:ok, pid} = GenServer.start_link(PastryNode, [index, 1, lesserNodes, greaterNodes, routeTable, []] , name: newnodeName)
            :global.register_name(newnodeName,pid)
            sendArrivalMessage(index-1, nodeId)
            generateActors(index+1, totalNodes, routingTable)
        end
    end
...
defmodule MasterNode do
    ...
    def handle_call(:track, _from, messages) do
		...
		IO.puts Enum.at(messages, 0)
		IO.puts Enum.at(messages, 1)
        IO.puts averagehops
...
defmodule Project3 do
  def main(args) do
    ...
    #IO.puts "Routing table is created"
    masternodeName = String.to_atom("nodemaster")
    ...
    PastryNode.generateActors(1, numNodes, routingTable)
